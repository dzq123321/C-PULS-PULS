...
四次握手：我们想的是利用对称密钥加密体制进行加密，速度快。但需要客户将对称加密的密钥发送给服务器，这个时候就需要非对称加密，再第一次建立TLS连接后，服务器将非对称加密的公钥发给云，云利用公钥将对称加密的密钥加密，随后发生给服务器，服务器之格式化利用非对称加密的私钥解密获得对称加密的私钥

解决报文可能遭篡改问题——数字签名？
网络传输过程中需要经过很多中间节点，虽然数据无法被解密，但可能被篡改，那如何校验数据的完整性呢？----校验数字签名。
将一段文本先用Hash函数生成消息摘要，然后用发送者的私钥加密生成数字签名，与原文文一起传送给接收者。接下来就是接收者校验数字签名的流程了。
cookie  session token 都是致力于保存用户和服务器之间登录状态的技术
cookie：是一种技术，session和token都使用了cookie
是一种KV类型消息，浏览器根据用户的信息生成cookie信息发送给浏览器进行保存，在之后的http连接时，浏览器在http报文中添加cookie信息（登录信息）。减少了验证开销
session：是cookie的升级版，用户的信息是临时存放在服务器中，而浏览器只存放一个sessionid，建立连接时浏览器发送seesionid到服务器，服务器根据sessionid进行判断（登录信息）。无法实现负载均衡
token：有点类似于https，是session的升级版，服务器不在存放session存放用户的临时信息（登录信息），而是在用户第一次登陆时利用hash函数和私钥计算token返回，随后用户每次登陆的时候携带token签名，在收到客户发送的而消息是计算token进行验证

其实token和session的主要区别在于服务器验证阶段，token是进行计算然后对比，而session则需要在数据库中进行查找。
```
